...
SPINUP_VAR_SUM = {"TOTSOMC":['totsomc'],#,'litr1c','litr2c','litr3c','cwdc','soil1c','soil2c','soil3c','soil4c'], 
                  "TOTSOMN":['litr1n','litr2n','litr3n','cwdn','soil1n','soil2n','soil3n','soil4n']}
...
def _load_forcing_matrix(
    ...
    for var in forcing_vars:
        if var not in ds.variables:
            continue
        #var_hourly = (ds[var].load().resample(time='1H').first()).convert_calendar('noleap', dim='time')
        var_hourly = (ds[var].coarsen(time=2).mean()).convert_calendar('noleap', dim='time')
        # print(var_hourly['time'])
...
def _restart_file(case, ens_num: int) -> Path:
    ...
    # print("Old path is: ")
    # print(rundir / f"{case.casename}.elm.r.{yst}-01-01-00000.nc")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--outputdir", default="/pscratch/sd/t/tianyihu/E3SM_out/SOIL_project", help="pklefile output directory")
    return parser
...
def main() -> int:
    ...
    print("Model output variable is:" )
    print(outvars)
    print("Raw forcing variables are:")
    print(forcing_vars)
    print("Spinup vars are:")
    print(spinup_vars)
    ...
    uq_out = outdir / "UQ_output" / case.casename / "surrogate_forcing"
