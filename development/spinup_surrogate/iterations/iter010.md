...
| 4 | The local `scancel` wrapper segfaulted/delayed while cancelling an erroneous submission. | Control-plane anomaly; cancellation was not timely enough to prevent the bad array from dispatching. | Prefer the site absolute `/usr/bin/scancel`; after any cancellation request, immediately reconcile `squeue` and accounting rather than assuming cancellation succeeded. |
