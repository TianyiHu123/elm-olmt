...
- Package state: `<not approved | approved | exhausted>`
...
- Locked dependencies, scope, exclusions, gates, and decision rule:
...
<Copy the complete planning-only proposal from the latest closed iteration report unchanged, or
record an explicit terminal declaration. For pre-kickoff Iter001, record the user-supplied plan or
state that none has been supplied.>
