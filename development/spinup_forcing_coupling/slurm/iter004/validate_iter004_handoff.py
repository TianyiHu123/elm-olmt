#!/usr/bin/env python3
"""Cross-validate Iter004 closeout records and non-circular commit identity."""
...
REPORT = COUPLING_ROOT / "iterations" / "iter004.md"
CUMULATIVE = COUPLING_ROOT / "ITERATION_SUMMARY.md"
REGISTRY = COUPLING_ROOT / "registry.csv"
CURRENT = COUPLING_ROOT / "handoff" / "CURRENT.md"
SUMMARY_ROOT = COUPLING_ROOT / "summaries" / "iter004"
DECISION_PATH = SUMMARY_ROOT / "iter004_decision.json"
ACCOUNTING_PATH = SUMMARY_ROOT / "iter004_accounting.csv"
SITE_MEDIANS_PATH = SUMMARY_ROOT / "iter004_site_metric_medians.csv"
OUTPUT_ROOT = (
    "/xdisk/chopinsong/tianyihu/E3SM_out/SOIL_project/"
    "spinup_forcing_coupling"
)
SUMMARY_PATH = "development/spinup_forcing_coupling/summaries/iter004"
OBJECTIVE = "Offline forcing versus coupled dual-variant ELM PPE SR comparison"
BOUNDED_SCOPE = (
    "Nine sites; offline + drop32 + drop21_corr080; 9×100 timeseries ON; "
    "four-figure plot package; no skill floor"
)
ACCEPTANCE_RESULT = "pass"
DECISION = (
    "Offline-versus-coupled comparison completed with metrics, timeseries, and "
    "plot package; predictive scores characterized; production MCMC readiness "
    "not established"
)
...
def validate_records(decision: dict[str, Any]) -> dict[str, str]:
    ...
    require("- Iteration ID: `iter004`" in report, "report iteration ID drift")
    ...
    require("- Active iteration: `iter004`" in current, "handoff iteration ID drift")
    ...
    require("## iter004" in cumulative, "cumulative summary lacks Iter004")
    ...
    with REGISTRY.open(encoding="utf-8", newline="") as stream:
        ...
        rows = list(reader)
    matches = [row for row in rows if row["iteration_id"] == "iter004"]
    require(len(matches) == 1, "registry must contain exactly one Iter004 row")
    row = matches[0]
    expected = {
        "status": "completed",
...
def validate_artifacts(decision: dict[str, Any]) -> None:
    ...
    expected = {
        ...
        "output_root": OUTPUT_ROOT,
        "summary_path": SUMMARY_PATH,
        "authoritative_preflight_job": "23515370",
        "full_array_job": "23515500",
        "authoritative_validate_job": "23515820",
        "closed_at": "2026-08-06T17:59:11-0700",
    }
    for key, value in expected.items():
...
def main() -> int:
    ...
    require(args.active_iteration_job_count == 0, "active Iter004 jobs remain")
    require(args.expected_parent == REPOSITORY_PARENT, "expected-parent argument drift")
    require(
        args.expected_subject == "Close Iter004 offline-versus-coupled comparison",
    ...
    print(
        "PASS: Iter004 records, artifacts, accounting, and "
