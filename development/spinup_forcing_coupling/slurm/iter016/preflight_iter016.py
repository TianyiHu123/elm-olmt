#!/usr/bin/env python3
"""Iter016 preflight: locked hashes, hybrid dry-checks, campaign reuse smoke."""
...
EXPECTED = {
    ...
    "abby_pool": "3627bb1df152e2f4356787a6634c96dfe533bc2ca55a30a7aa90fc4d9fd50592",
    "jerc_pool": "40ac807e17803316b1200b7caa316d2ee45dde3a82fa1570345b3da4e282e4df",
...
CAMPAIGN = {
    "ABBY": {"resolution": "daily", "de_scale": "0.50"},
    "JERC": {"resolution": "hourly", "de_scale": "0.75"},
}
LEDGER_RESOLUTION = {"ABBY": "daily", "JERC": "hourly"}
SEEDS = list(range(9009, 9018))
...
COMPILE_TARGETS = (
    ...
    SLURM / "preflight_iter016.py",
    SLURM / "analyze_iter016.py",
    SLURM / "validate_iter016_handoff.py",
    TOOLS / "ensemble_common.py",
    TOOLS / "ensemble_seed_health.py",
    TOOLS / "ensemble_map_inventory.py",
    TOOLS / "ensemble_equifinality_diagnostics.py",
...
def main() -> int:
    ...
    parser.add_argument("--abby-pool-reference", required=True, type=Path)
    parser.add_argument("--jerc-pool-reference", required=True, type=Path)
    ...
    require(sha256(args.abby_pool_reference) == EXPECTED["abby_pool"], "ABBY pool reference mismatch")
    require(sha256(args.jerc_pool_reference) == EXPECTED["jerc_pool"], "JERC pool reference mismatch")
    require("hybrid_high_l_maximin" in POOL_RULES, "hybrid pool rule missing")
    ...
    for site in ("ABBY", "JERC"):
        ...
        targets[f"{site.lower()}_{resolution}"] = {"sha256": target["identity"]["sha256"]}
        ...
        pool, pool_logp, pool_strata, diagnostics = choose_candidate_pool(
            np.asarray(ledger["states"], dtype=float),
            np.asarray(ledger["log_posterior"], dtype=float),
        ...
        indices, selected = select_production_walkers(
            ...
            target["pmax"],
            seed=SEEDS[0],
            log_posterior=target["log_posterior"],
            walker_count=64,
            require_stored_posterior_match=bool(reuse["require_stored_posterior_match"]),
        )
        require(selected.shape == (64, 15), f"{site} walker shape")
        reuse_checks[site.lower()] = {
            "resolution": resolution,
            "de_scale": campaign["de_scale"],
            "n_seeds": len(SEEDS),
    ...
    payload = {
        "schema": "spinup-forcing-coupling-iter016-preflight-v1",
        "pool_rule": "hybrid_high_l_maximin",
        "pool_reuse_policy": "site_hybrid_pool_reuse_v1",
        "seeds": SEEDS,
        "targets": targets,
    ...
    print("PREFLIGHT_PASS sites=ABBY,JERC seeds=9009-9017")
    return 0
