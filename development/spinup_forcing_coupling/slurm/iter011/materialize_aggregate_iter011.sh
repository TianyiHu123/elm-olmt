#!/usr/bin/env bash
# Materialize the one authorized Iter011 aggregate/validation work unit after campaign terminal accounting.
set -euo pipefail
...
test -d "${AGGREGATE_DIR}"; test -z "$(find "${AGGREGATE_DIR}" -mindepth 1 -maxdepth 1 -print -quit)"; test ! -e "${SUMMARY_ROOT}"
...
echo AGGREGATE_MATERIALIZE_PASS
