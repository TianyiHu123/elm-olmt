#!/usr/bin/env bash
# Preserve failed aggregate v1/v2 materials, then make the approved v3 POSIX launcher correction.
set -euo pipefail
...
readonly ARCHIVE_VERSION="${ARCHIVE_VERSION:-v2}"
test -f "${AGGREGATE_DIR}/identity_record.env"; test ! -e "${AGGREGATE_DIR}/identity_record_${ARCHIVE_VERSION}.env"; test ! -e "${SUMMARY_ROOT}"
for name in source_manifest.sha256 dependency_manifest.sha256 submit_aggregate_iter011.slurm; do
  cp "${AGGREGATE_DIR}/${name}" "${AGGREGATE_DIR}/${name%.*}_${ARCHIVE_VERSION}.${name##*.}"
done
for name in submission_config.env canonical_submission_config.env identity_record.env; do
  cp "${AGGREGATE_DIR}/${name}" "${AGGREGATE_DIR}/${name%.env}_${ARCHIVE_VERSION}.env"
done
...
echo AGGREGATE_REMATERIALIZE_V3_PASS
