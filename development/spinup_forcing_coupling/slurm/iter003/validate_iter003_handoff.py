#!/usr/bin/env python3
"""Cross-validate Iter003 closeout records and non-circular commit identity."""
...
REPORT = COUPLING_ROOT / "iterations" / "iter003.md"
CUMULATIVE = COUPLING_ROOT / "ITERATION_SUMMARY.md"
REGISTRY = COUPLING_ROOT / "registry.csv"
CURRENT = COUPLING_ROOT / "handoff" / "CURRENT.md"
SUMMARY_ROOT = COUPLING_ROOT / "summaries" / "iter003"
DECISION_PATH = SUMMARY_ROOT / "iter003_decision.json"
ACCOUNTING_PATH = SUMMARY_ROOT / "iter003_accounting.csv"
SITE_MEDIANS_PATH = SUMMARY_ROOT / "iter003_site_metric_medians.csv"
OUTPUT_ROOT = (
    "/xdisk/chopinsong/tianyihu/E3SM_out/SOIL_project/"
    "spinup_forcing_coupling"
)
SUMMARY_PATH = "development/spinup_forcing_coupling/summaries/iter003"
OBJECTIVE = "Coupled spinup–forcing dual-variant ELM PPE SR comparison"
BOUNDED_SCOPE = (
    "Nine sites; both spinup variants; ABBY×5 pilot timeseries ON; "
    "9×100×both full timeseries OFF; MCMC-ready CLI; ELM compare; no skill floor"
)
ACCEPTANCE_RESULT = "pass"
DECISION = (
    "Executable dual-variant coupled path demonstrated with ELM comparison "
    "evidence; predictive scores characterized; production MCMC readiness not "
    "established"
)
FORCING_SHA = "8d139b32473eebe3f75f77042e542f49ec3c80e89bc65c76b2e98a5c70f4553e"
DROP32_SHA = "56bbd151103add74b5a0794e8d1bf4496c186d3a72e70b1b65c5ab247abd317e"
DROP21_SHA = "1427dc565af858e9c089a5b9545a7f127e42789ef1fe5c9af3af7f8cb12a3023"
...
def validate_records(decision: dict[str, Any]) -> dict[str, str]:
    ...
    require("- Iteration ID: `iter003`" in report, "report iteration ID drift")
    ...
    require("- Active iteration: `iter003`" in current, "handoff iteration ID drift")
    ...
    require("## iter003" in cumulative, "cumulative summary lacks Iter003")
    ...
    with REGISTRY.open(encoding="utf-8", newline="") as stream:
        ...
        rows = list(reader)
    matches = [row for row in rows if row["iteration_id"] == "iter003"]
    require(len(matches) == 1, "registry must contain exactly one Iter003 row")
    row = matches[0]
    expected = {
        "status": "completed",
    ...
    for value in (FORCING_SHA, DROP32_SHA, DROP21_SHA):
...
def validate_artifacts(decision: dict[str, Any]) -> None:
    ...
    expected = {
        ...
        "drop32_sha256": DROP32_SHA,
        "drop21_corr080_sha256": DROP21_SHA,
        "output_root": OUTPUT_ROOT,
        "summary_path": SUMMARY_PATH,
        "authoritative_preflight_job": "23510375",
        "authoritative_pilot_job": "23510419",
        "full_array_job": "23510434",
        "authoritative_validate_job": "23510503",
        "closed_at": "2026-08-05T20:30:09-0700",
    }
    for key, value in expected.items():
...
def main() -> int:
    ...
    require(args.active_iteration_job_count == 0, "active Iter003 jobs remain")
    require(args.expected_parent == REPOSITORY_PARENT, "expected-parent argument drift")
    require(
        args.expected_subject == "Close Iter003 coupled spinup-forcing ELM comparison",
        "expected-subject argument drift",
    )
    ...
    print(
        "PASS: Iter003 records, artifacts, accounting, and "
