#!/usr/bin/env python3
"""Evaluate one fixed-length Iter012 site without converting inconclusive tau into failure."""
...
def split_seed_walker_chains(chains: np.ndarray) -> np.ndarray:
    """Return split seed/walker trajectories with shape (chain, draw, parameter)."""
    nseed, nstep, nwalker, ndim = chains.shape
    half = nstep // 2
    return (
        chains[:, : 2 * half]
...
def multi_chain_ess(values: np.ndarray) -> float:
    """Geyer initial-positive-sequence ESS for equal-length chains."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--evidence-role", required=True, choices=["canonical", "legacy"]
    )
    parser.add_argument("--required-move-configuration", default="de_mixture")
    parser.add_argument("--allow-move-mismatch", action="store_true")
...
def main() -> int:
    ...
    final_output = args.output / "artifacts"
    staging_output = args.output / ".artifacts.build"
    ...
    expected_pool_sha256 = sha256(args.pool)
    expected_source_sha256 = sha256(args.source_manifest)
    expected_dependency_sha256 = sha256(args.dependency_manifest)
    archives = []
    acceptance = []
    observed_moves = []

    for seed in SEEDS:
        leaf = args.production_root / f"seed_{seed}"
        ...
        required_steps = list(range(8000, NSTEPS + 1, 8000))
        if (
        ...
        if (
            ...
            or production.get("site") != args.site
            or production.get("resolution") != args.resolution
            ...
            or production.get("repository_commit") != args.repository_commit
            or production.get("source_manifest_sha256") != expected_source_sha256
            or production.get("dependency_manifest_sha256")
            != expected_dependency_sha256
            or production.get("sampler_coordinates", raw_metadata.get("transform", {}).get("coordinate_system"))
            != "transformed"
        ...
        if (
            selection.get("site") != args.site
            or selection.get("resolution") != args.resolution
        ...
        if args.evidence_role == "canonical":
            ...
            if (
                ...
                or selection.get("validation_sha256")
                != selection_validation_sha256(selection)
            ):
                raise ValueError(f"{leaf}: canonical selection validation incomplete")
        provenance = raw_metadata.get("provenance", {})
        if (
            raw_metadata.get("seed") != seed
            or raw_metadata.get("sites") != [args.site]
            or raw_metadata.get("nwalkers") != NWALKERS
            or raw_metadata.get("nsteps") != NSTEPS
            or provenance.get("REPOSITORY_COMMIT") != args.repository_commit
    ...
    move_matches_contract = all(
        move == args.required_move_configuration for move in observed_moves
    )
    if not move_matches_contract and not args.allow_move_mismatch:
        raise ValueError(
            f"production move mismatch: observed={observed_moves}, "
            f"required={args.required_move_configuration}"
        )
    if args.evidence_role == "canonical" and args.allow_move_mismatch:
        raise ValueError("canonical evaluation cannot allow a sampler-move mismatch")
    ...
    for chain, _, _ in archives:
        ...
        except Exception as exc:  # noqa: BLE001 - inconclusive is a valid result
    ...
    flat_log_probability = np.concatenate(
        [
            archive[2][descriptive_discard:].reshape(-1)
    ...
    rng = np.random.default_rng(12012 if args.site == "ABBY" else 12013)
    sampled_states = flat[
        rng.choice(len(flat), size=min(64, len(flat)), replace=False)
    ]
    observation = target["obs"][args.site]["SR"]
    error = target["obs_err"][args.site]["SR"]
    median_prediction = target["predict"](args.site, median_state)
    map_prediction = target["predict"](args.site, map_state)
    predictions = np.asarray(
        [target["predict"](args.site, state) for state in sampled_states]
    )
    predictive = {
    ...
    qualified = bool(
        args.evidence_role == "canonical"
        and move_matches_contract
    ...
    label = (
        "diagnostically_qualified" if qualified else "fixed_length_inconclusive"
    )
    if args.evidence_role == "legacy" and not move_matches_contract:
        label = "legacy_misconfigured_sampler"

    result = {
        "schema": "spinup-forcing-coupling-iter012-evaluation-v2",
        "evidence_role": args.evidence_role,
        "site": args.site,
        "resolution": args.resolution,
        "seeds": list(SEEDS),
        "chain_shape": list(chains.shape),
        "required_move_configuration": args.required_move_configuration,
        "observed_move_configurations": observed_moves,
        "move_matches_contract": move_matches_contract,
        "mean_acceptance_by_seed": np.mean(acceptance_array, axis=1).tolist(),
        ...
        "production_repository_commit": args.repository_commit,
        "production_source_manifest_sha256": expected_source_sha256,
        "production_dependency_manifest_sha256": expected_dependency_sha256,
        "status": "pass",
    }
    np.savez_compressed(
        args.output / "physical_traces.npz",
        traces=post,
        log_posterior=flat_log_probability,
    )

    corner_rng = np.random.default_rng(12012 if args.site == "ABBY" else 12013)
    corner = flat[
        corner_rng.choice(len(flat), size=min(2000, len(flat)), replace=False)
    ]
    ...
    figure.tight_layout()
    figure.savefig(args.output / "physical_corner.png", dpi=150)
    plt.close(figure)

    with (args.output / "hourly_predictions.csv").open(
    ...
    print(
        f"EVALUATION_PASS role={args.evidence_role} "
        f"site={args.site} label={label}"
    )
    return 0
