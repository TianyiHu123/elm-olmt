#!/usr/bin/env python3
"""Cross-validate Iter007 closeout records and non-circular commit identity."""
...
REPORT = COUPLING_ROOT / "iterations" / "iter007.md"
CUMULATIVE = COUPLING_ROOT / "ITERATION_SUMMARY.md"
REGISTRY = COUPLING_ROOT / "registry.csv"
CURRENT = COUPLING_ROOT / "handoff" / "CURRENT.md"
SUMMARY_ROOT = COUPLING_ROOT / "summaries" / "iter007"
DECISION_PATH = SUMMARY_ROOT / "iter007_decision.json"
ACCOUNTING_PATH = SUMMARY_ROOT / "iter007_accounting.csv"
...
BOUNDED_SCOPE = "ABBY+JERC joint; coupled drop21_corr080; SR; 64x500; flat campaign layout; suggested diagnostics; integrity-only"
...
REPOSITORY_PARENT = "4051e875bff93742bdf5ccfb69a94a9ce10468c1"
EXPECTED_SUBJECT = "Close Iter007 joint ABBY+JERC coupled MCMC campaign"
NEXT_PLAN_MARKER = "Proposed iteration: `iter008`"
...
def validate_records(decision: dict[str, Any]) -> dict[str, str]:
    ...
    require("- Iteration ID: `iter007`" in report, "report iteration ID drift")
    ...
    require("- Active iteration: `iter007`" in current, "handoff iteration ID drift")
    ...
    require("## iter007" in cumulative, "cumulative summary lacks Iter007")
    ...
    with REGISTRY.open(encoding="utf-8", newline="") as stream:
        ...
        rows = list(reader)
    matches = [row for row in rows if row["iteration_id"] == "iter007"]
    require(len(matches) == 1, "registry must contain exactly one Iter007 row")
    row = matches[0]
    expected = {
        "status": "completed",
...
def validate_artifacts(decision: dict[str, Any]) -> None:
    ...
    for name in (
        "iter007_skill_table.csv",
        "iter007_chain_health.json",
        "iter007_collocation_audit.csv",
        "iter007_delta_logL.csv",
        "iter007_posterior_summary.csv",
    ):
    ...
    require(decision.get("chain_health", {}).get("flat_samples") == 5120, "flat samples drift")
...
def validate_git_identity(
    *,
    ...
    if phase == "postcommit":
        ...
        # Allow only ignored/untracked noise outside controlled paths.
        status = git("status", "--porcelain")
        dirty = []
        for line in status.splitlines():
            ...
            if path.endswith(".swp"):
                continue
            dirty.append(path)
        require(not dirty, f"postcommit worktree dirty: {dirty}")
        return
...
def main() -> int:
    ...
    print(
        "PASS: Iter007 records, artifacts, accounting, and "
