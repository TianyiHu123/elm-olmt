...
def main():
    ...
    print('PTCLM site directory: '+ptclminfodata)
    ...
    if runtype == 'site':
        ...
        case_names = cfg['simulation']['case_names']
    ...
    # PHS options
    use_hydrstress  = cfg['biogeochemistry'].get('use_hydrstress', False)
    ...
    nyear_spinup_cycle = cfg['run_lengths'].get('nyear_spinup_cycle', 5)
    ...
    if (use_hydrstress):
        print('Use phs is true', use_hydrstress)
        compset_base=compset_base+'PHS'
    if (use_fates):
    ...
    for c in range(0,len(compsets)):
        ...
        if '1850' in compsets[c]:
            print('Cycling ', str(nyear_spinup_cycle) ,' years of forcing data')
    ...
    for site in sites:
      ...
      for c in range(0,ncases):
        ...
        if (metdir != ''):
            cases[c].get_forcing(mettype=mettype, metdir=metdir, spinup_cycle=nyear_spinup_cycle)
        else:
        ...
        if (c == 0):
            ...
            print('Get the surface and domain data')
